...
def _default_reward_cfg(**overrides):
    ...
    cfg = dict(
        scales={
            ...
            "base_height": -2.0,
            "torques": -5.0e-6,
            # Exercise feet/contact/dof terms and their sensors.
...
def test_ref_dof_leg_is_l1_exponential():
    """The leg ref_dof reward uses exp(-L1_err * 0.1), not L2/sigma_force."""
...
def test_clip_actions_and_obs_bound_runaway():
    """±100 clamps (clip_actions / clip_observations): a blown-up policy
    (huge actions) must not poison the obs (last_actions block) or the action_rate
    reward — the safety valve that prevents a feedback runaway driving training to
    value-loss=inf and the std>=0 crash."""
    _skip_if_no_mujoco()
    env = _make_env(num_envs=2)
...
def test_termination_uses_unifp_roll_pitch_thresholds():
    """Termination uses |pitch| > 1.0 rad or |roll| > 0.8 rad thresholds.

    Using ``gravity_z <= 0`` (~90 deg tip on either axis) would let the robot fall
    far past the per-axis Euler limits before terminating and change the
    survivable-state set / ``alive`` reward integral.
    """
    from types import SimpleNamespace
