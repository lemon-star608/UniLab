"""Unit tests for SimToolReal observation assembly.

The tests cover the task observation contract:
...
def _make_env(n: int = N_ENVS, disable_dr: bool = False) -> MagicMock:
    ...
    env._obs_queue = np.zeros(
        (n, max(cfg.domain_randomization.obs_delay_max, 1), obs_d), dtype=np.float32
    )
    ...
    if disable_dr:
        ...
        env.cfg.reward_config.object_base_size = cfg.reward_config.object_base_size
...
class TestBuildObservations:
    ...
    def test_joint_pos_normalized(self, env, state) -> None:
        ...
        np.testing.assert_allclose(
            jp_actor, 0.0, atol=1e-5, err_msg="joint_pos at midpoint should normalise to 0"
        )
    ...
    def test_quat_w_at_end_actor(self, env, state) -> None:
        ...
        np.testing.assert_allclose(
            palm_rot[:, 2], sin45, atol=1e-5, err_msg="palm_rot z component should be sin(45)"
        )
    ...
    def test_dstar_sentinel_preserved(self, env, state) -> None:
        ...
        """
        sentinel_val = -1.0
        state.info["closest_keypoint_max_dist"] = np.full((N_ENVS,), sentinel_val, dtype=np.float32)
        out = build_observations(env, state)
    ...
    def test_output_dtype_float32(self, env, state) -> None:
        """Both output arrays must be float32."""
        out = build_observations(env, state)
    ...
    def test_progress_log_scaling(self, env, state) -> None:
        ...
        np.testing.assert_allclose(
            prog, 0.0, atol=1e-5, err_msg="progress should be log(0/10+1)=0 at step 0"
        )

    def test_reward_feature_scales_raw_reward_for_critic_only(self, env, state) -> None:
        """Critic feature applies 0.01 while the task info remains raw."""
        raw_reward = 1.0
        ...
        np.testing.assert_allclose(
            ...
            raw_reward * 0.01,
            atol=1e-5,
            err_msg="critic reward feature must be raw reward times 0.01",
        )
...
class TestBuildResetObservations:
    """Row-restricted sibling used by the DR provider's reset path."""

    @staticmethod
    def _row_info(state: MagicMock, rows: np.ndarray) -> dict[str, np.ndarray]:
        """Slice a full-batch info dict down to ``rows``, as the provider does."""
        return {key: np.asarray(value)[rows] for key, value in state.info.items()}

    def test_returns_only_selected_rows(self) -> None:
        env = _make_env(N_ENVS, disable_dr=True)
        state = _make_state(N_ENVS)
        rows = np.array([1, 3], dtype=np.intp)

        out = build_reset_observations(env, rows, self._row_info(state, rows))

        assert out["obs"].shape == (2, compute_obs_dim(env.cfg.obs.obs_list))
        assert out["critic"].shape == (2, compute_obs_dim(env.cfg.obs.state_list))

    def test_equals_full_batch_rows(self) -> None:
        """Row-restricted assembly must be numerically identical to slicing the full batch."""
        env = _make_env(N_ENVS, disable_dr=True)
        state = _make_state(N_ENVS)

        # Per-row backend values so a wrong row selection cannot pass by accident.
        offsets = np.arange(N_ENVS, dtype=np.float32)[:, None, None]
        env._backend.get_body_pos_w.side_effect = lambda ids: (
            np.zeros((N_ENVS, len(np.atleast_1d(ids)), 3), dtype=np.float32) + offsets
        )
        env.get_joint_pos_canon.return_value = (
            np.zeros((N_ENVS, NUM_JOINTS), dtype=np.float32) + offsets[:, :, 0]
        )
        state.info["prev_targets"] = (
            np.zeros((N_ENVS, NUM_JOINTS), dtype=np.float32) + offsets[:, :, 0]
        )
        state.info["goal_pos"] = np.zeros((N_ENVS, 3), dtype=np.float32) + offsets[:, :, 0]

        full = build_observations(env, state)
        rows = np.array([2, 0], dtype=np.intp)  # deliberately out of order
        subset = build_reset_observations(env, rows, self._row_info(state, rows))

        for group in ("obs", "critic"):
            np.testing.assert_allclose(subset[group], full[group][rows], rtol=0, atol=0)

    def test_missing_tracker_raises(self) -> None:
        """Missing fresh trackers must raise, not silently fall back to stale values."""
        env = _make_env(N_ENVS, disable_dr=True)
        state = _make_state(N_ENVS)
        rows = np.array([0], dtype=np.intp)
        info = self._row_info(state, rows)
        del info["goal_quat"]

        with pytest.raises(KeyError, match="goal_quat"):
            build_reset_observations(env, rows, info)

    def test_delay_queues_only_touch_selected_rows(self) -> None:
        """With the delays enabled, non-selected rows keep their queue history."""
        env = _make_env(N_ENVS)  # stock cfg: use_obs_delay / object-state delay on
        state = _make_state(N_ENVS)
        env._obs_queue[:] = 3.0
        env._object_state_queue[:] = 3.0

        rows = np.array([2], dtype=np.intp)
        build_reset_observations(env, rows, self._row_info(state, rows))

        keep = [0, 1, 3]
        np.testing.assert_allclose(env._obs_queue[keep], 3.0, atol=1e-6)
        np.testing.assert_allclose(env._object_state_queue[keep], 3.0, atol=1e-6)
        # Selected row is flushed: every slot holds the same fresh frame.
        slots = env._obs_queue[2]
        np.testing.assert_allclose(slots, np.broadcast_to(slots[0], slots.shape), atol=1e-6)
        assert not np.allclose(slots[0], 3.0)
