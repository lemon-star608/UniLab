"""Interactive MuJoCo viewer for the Go2+Arm force-position task (CSE-PPO).

Opens a live MuJoCo window with a single Go2+arm robot on the same flat ground
used during play, drives it with a trained CSE-PPO policy, and lets you teleop it
from the keyboard:

  * Go2 base velocity  : forward/back, strafe left/right, turn left/right
...
The EE target is initialized at the spherical point ``[0.25, 0.68, 0.0]`` (the
closest in-range point to the reset pose) and can be nudged live.
...
# Magnitudes match training (ext gripper ±15 N, ext base ±20 N). A sudden
# step-impulse is out-of-distribution — the concurrent estimator integrates
# ~0.64 s of history, so a sharp spike just shoves the robot before the policy
# can react. The per-body ramp/hold step counts are read from the env config in
# ``_push_timing_from_env`` so they stay in sync with the training settings.
_IMPULSE_EE_N = 15.0
_IMPULSE_BASE_N = 20.0
...
class TeleopState:
    ...
    def reset(self) -> None:
        self.base_vel[:] = 0.0
        self.ee_sphere[:] = np.asarray(_EE_INIT_SPHERE, dtype=np.float64)
        self.clear_forces()
    ...
    def push_ee(self, axis: int, sign: float) -> None:
        self._ee_target[:] = 0.0
        self._ee_target[axis] = sign * _IMPULSE_EE_N
        self._ee_step = 0

    def push_base(self, axis: int, sign: float) -> None:
        self._base_target[:] = 0.0
        self._base_target[axis] = sign * _IMPULSE_BASE_N
        self._base_step = 0
...
def play_interactive(cfg: DictConfig, device: str) -> None:
    ...
    teleop = TeleopState(ranges=_ranges_from_env(env), **_push_timing_from_env(env))
