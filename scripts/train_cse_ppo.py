...
def play_cse_ppo(cfg: DictConfig, device: str) -> str | None:
    ...
    if "actor_state_dict" not in _ckpt_keys:
        print(
            f"Checkpoint at {load_path} is not a CSE-PPO checkpoint "
...
def main(cfg: DictConfig) -> None:
    ...
    try:
        if not cfg.training.play_only:
            ...
            wrapped_env = RslRlVecEnvWrapper(env, device=device)
            rl_cfg = _algo_config_dict(cfg)
            runner = CSEOnPolicyRunner(wrapped_env, rl_cfg, log_dir=log_dir, device=device)
        ...
        if cfg.training.play_only or not cfg.training.no_play:
            play_video_path = play_cse_ppo(cfg, device)
            if tracker is not None:
