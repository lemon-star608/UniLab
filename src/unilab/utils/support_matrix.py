...
# Code #10 M0-dev evidence is intentionally narrower than maintainer-confirmed
# release support.  It promotes exactly one dev-only combination to Tested.
...
def render_support_matrix(root: Path | None = None) -> str:
    ...
    lines = [
        ...
        "",
        "`RL-Games SAPG` / `simtoolreal` / MuJoCo 的 `Tested` 仅代表 Code #10 的 "
        "M0-dev provisional 证据，固定为 `mujoco-uni-runtime==0.4.0.dev0`。它不是正式 "
