"""Observation assembly for SimToolReal.
...
The reset path needs the same assembly for a **subset** of rows, so both entry
points share :func:`_assemble_observations`::

    def build_reset_observations(env, env_ids, reset_info) -> dict[str, np.ndarray]

That is a sibling, not a change to the frozen signature above: ``env_ids`` selects
the rows, and ``reset_info`` carries the reset trackers that ``state.info`` does
not hold yet at that point in the reset sequence.

All arrays are NumPy ``float32`` on CPU.
...
:func:`compute_keypoints` (contract §4.3).

Delay queues use :mod:`.delay_buffer` directly. Missing owner code is an import
failure; observation assembly never silently falls back to no-delay behavior.
...
``isaacsimenvs/tasks/simtoolreal/utils/``):
    ...
    reward feature is the raw reward multiplied by 0.01 for the critic only
...
from .delay_buffer import push_and_sample_delay, push_and_sample_delay_rows
...
def _episode_start_from_info(info: dict[str, np.ndarray]) -> np.ndarray:
    ...
    """
    steps = np.asarray(info["steps"], dtype=np.int32)
    successes = np.asarray(info["successes"], dtype=np.int32)
    return (steps == 0) & (successes == 0)


def _episode_start(state: NpEnvState) -> np.ndarray:
    """``_episode_start_from_info`` for a full :class:`NpEnvState`."""
    return _episode_start_from_info(state.info)
...
def _apply_object_state_dr(
    ...
    rows: np.ndarray | None = None,  # (n,) env indices, None == all envs
    ...
    Args:
        ...
        obj_pos: Object positions for the selected rows.
        obj_rot_wxyz: Object orientations (wxyz) for the selected rows.
        obj_linvel: Object linear velocities for the selected rows.
        obj_angvel: Object angular velocities for the selected rows.
        flush: Episode-start mask for the selected rows.
        rows: Env indices the inputs belong to, or ``None`` for the full batch.
            When given, only those rows of ``env._object_state_queue`` are
            touched — the running envs keep the history ``update_state`` pushed.
    ...
    dtype = env._np_dtype
    n = int(obj_pos.shape[0])

    # Pack state frame: pos(3) + quat(4) + linvel(3) + angvel(3) = 13
    state_frame = np.concatenate([obj_pos, obj_rot_wxyz, obj_linvel, obj_angvel], axis=-1).astype(
        dtype
    )

    # Push into rolling queue and sample delayed frame
    if rows is None:
        env._object_state_queue, delayed = push_and_sample_delay(
            env._object_state_queue, state_frame, env, flush=flush
        )
    else:
        env._object_state_queue, delayed = push_and_sample_delay_rows(
            env._object_state_queue, state_frame, rows, flush=flush
        )

    # Unpack delayed frame
    noisy_pos = delayed[:, 0:3] + (
        np.random.randn(n, 3).astype(dtype) * np.float32(dr.object_state_xyz_noise_std)
    )
...
def _apply_obs_delay(
    ...
    Args:
        env: Owning env instance.
        flush: Episode-start mask for the selected rows.
        policy_tensor: Actor observation for the selected rows.
        rows: Env indices the inputs belong to, or ``None`` for the full batch.

    Returns:
        The delayed actor observation, shape ``(n, D_actor)``.
    """
    if rows is None:
        ...
        env._obs_queue, delayed = push_and_sample_delay_rows(
            env._obs_queue, policy_tensor, rows, flush=flush
        )
    return delayed
...
def _get_scale_multiplier(env: Any) -> np.ndarray:
    ...
    the default DR range of ``(1.0, 1.0)``.  The DR provider resamples
...
def build_observations(
    ...
    """
    return _assemble_observations(env, state.info, rows=None)


def build_reset_observations(
    env: Any,
    env_ids: np.ndarray,
    reset_info: dict[str, np.ndarray],
) -> dict[str, np.ndarray]:
    """Assemble post-reset observations for ``env_ids`` only.

    Row-restricted sibling of :func:`build_observations`; the frozen §4.4
    signature above is untouched. Exists because the reset path has to return
    exactly ``len(env_ids)`` rows and must read the **fresh** tracker values,
    which at that moment live in the DR provider's ``info_updates`` and have not
    yet been scattered into ``state.info`` (np_env.py:287-297 runs after
    ``build_reset_observation``).

    Both quantities the source reads post-reset are already correct when this is
    called: ``backend.set_state`` ran before it (dr/manager.py:52,70), so the
    physics state is the reset state, and ``reset_info`` carries the reset
    trackers.

    Args:
        env: Owning env instance.
        env_ids: Env indices being reset, shape ``(k,)``.
        reset_info: Info values **already sliced to** ``env_ids`` order, one entry
            per key :func:`build_observations` reads. Missing keys raise rather
            than silently falling back to the stale ``state.info`` — a stale
            observation is harder to spot than a zero one.

    Returns:
        ``{"obs": (k, num_actor_obs), "critic": (k, num_critic_obs)}``.

    Notes:
        Delay queues are flushed for these rows (``flush=True`` throughout, since
        every row here starts a fresh episode), so the frame this returns is the
        post-reset frame regardless of the drawn delay, and the terminal frame
        ``update_state`` pushed earlier in the same step is overwritten. That
        leaves the queues in the state the source has after its own post-reset
        ``_get_observations`` (obs_utils.py:215 ``_episode_start`` flush).
    """
    rows = np.asarray(env_ids, dtype=np.intp)
    missing = [key for key in _RESET_INFO_KEYS if key not in reset_info]
    if missing:
        raise KeyError(
            "build_reset_observations needs fresh reset values for "
            f"{sorted(missing)}; got keys {sorted(reset_info)}"
        )
    return _assemble_observations(env, reset_info, rows=rows)


# Info keys read by _assemble_observations. The reset path must supply every one
# of them from the reset plan; see build_reset_observations.
_RESET_INFO_KEYS: tuple[str, ...] = (
    "prev_targets",
    "goal_pos",
    "goal_quat",
    "object_scales",
...
def _assemble_observations(
    ...
    """Shared observation assembly for the step and reset paths.

    Args:
        env: Owning env instance.
        info: Info mapping. When ``rows is None`` these are the full ``(N, ...)``
            arrays from ``state.info``; otherwise they are already sliced to
            ``rows`` order and have leading dimension ``len(rows)``.
        rows: ``None`` for the whole batch, else the env indices to build for.
            Backend getters always return all ``N`` rows (that is the contract),
            so they are indexed down right after the call; the arithmetic and the
            delay-queue updates then run on ``len(rows)`` rows only.

    Returns:
        ``{"obs": ..., "critic": ...}`` with ``len(rows)`` (or ``N``) rows.
    """
    dtype = env._np_dtype
    n = env._num_envs if rows is None else int(rows.shape[0])
    dr = env.cfg.domain_randomization
    rew_cfg = env.cfg.reward_config
    goal_cfg = env.cfg.goal

    # Row selector applied to every full-batch backend read below. `slice(None)`
    # keeps the step path a plain view with no copy.
    sel: slice | np.ndarray = slice(None) if rows is None else rows

    # ------------------------------------------------------------------ #
    # 1. Joint state (canonical order)  obs_utils.py:136-145             #
    # ------------------------------------------------------------------ #
    joint_pos_raw = env.get_joint_pos_canon()[sel]  # (n, 29)
    joint_vel = env.get_joint_vel_canon()[sel]  # (n, 29)
    ...
    # `info` is already row-aligned (see the `rows` docstring), so no [sel] here.
    ...
    palm_pos_w = env._backend.get_body_pos_w(palm_ids)[sel, 0, :]  # (n, 3)
    palm_rot_wxyz = env._backend.get_body_quat_w(palm_ids)[sel, 0, :]  # (n, 4) wxyz
    palm_linvel = env._backend.get_body_lin_vel_w(palm_ids)[sel, 0, :]  # (n, 3)
    palm_angvel = env._backend.get_body_ang_vel_w(palm_ids)[sel, 0, :]  # (n, 3)
    ...
    ft_pos_w = env._backend.get_body_pos_w(ft_ids)[sel]  # (n, 5, 3)
    ft_rot_w = env._backend.get_body_quat_w(ft_ids)[sel]  # (n, 5, 4) wxyz
    ...
    obj_pos = env._backend.get_body_pos_w(obj_ids)[sel, 0, :]  # (n, 3)
    obj_rot_wxyz = env._backend.get_body_quat_w(obj_ids)[sel, 0, :]  # (n, 4) wxyz
    obj_linvel = env._backend.get_body_lin_vel_w(obj_ids)[sel, 0, :]  # (n, 3)
    obj_angvel = env._backend.get_body_ang_vel_w(obj_ids)[sel, 0, :]  # (n, 3)
    ...
    # ------------------------------------------------------------------ #
    # On the reset path every row is by definition an episode start, and the
    # steps/successes it would be derived from are the freshly seeded zeros.
    flush = _episode_start_from_info(info) if rows is None else np.ones((n,), dtype=bool)
    if dr.use_object_state_delay_noise:
        noisy_obj_pos, noisy_obj_rot_wxyz, noisy_obj_vel = _apply_object_state_dr(
            env, obj_pos, obj_rot_wxyz, obj_linvel, obj_angvel, flush, rows=rows
        )
    else:
    ...
    scale_mult = _get_scale_multiplier(env)[sel]  # (n, 3) ≈ 1.0 by default
    ...
    # obs_utils.py:183-189 (the task's d* sentinel contract)
    ...
    # Source-native task rewards remain raw at the env boundary. The critic
    # privileged feature has its own 0.01 normalization; the learner's native
    # reward shaper applies its separate 0.01 scale after env output.
    reward_feat = (np.asarray(info["reward"], dtype=dtype) * np.float32(0.01))[:, None]
    ...
    if dr.joint_velocity_obs_noise_std > 0:
        ...
        obs_noisy["joint_vel"] = joint_vel + noise * np.float32(dr.joint_velocity_obs_noise_std)
    ...
    if dr.use_obs_delay:
        policy_tensor = _apply_obs_delay(env, flush, policy_tensor, rows=rows)
...
__all__ = ["build_observations", "build_reset_observations"]
