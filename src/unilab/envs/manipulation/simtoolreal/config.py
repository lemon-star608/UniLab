...
class AssetsCfg:
    ...
    Source: ``AssetsCfg`` (cfg:56-101). The URDF paths are kept as provenance for
    the MJCF assets built by ``unilab.tools.build_simtoolreal_assets``; the
    frictions are baked into the MJCF at asset-build time, not randomized per
    reset (MIGRATION_00 decision D5).
    """

    # Source URDFs the MJCF assets are generated from (cfg:57-60).
    robot_urdf: str = (
    ...
    # Object pool selection (cfg:72-94). T0 loads a single fixed hammer; the
    # 1200-object pool is T8.
    ...
    # Static per-material frictions, set once at asset creation (cfg:97-101).
...
class ResetCfg:
    ...
    Goal-sampling fields from the same source class are regrouped onto
    :class:`GoalCfg`; what remains here is the object / joint / table reset
    distribution. T0 only consumes ``table_reset_z`` and
    ``table_object_z_offset`` (for the default object height); the sampling
    itself is T4.

    .. note::
       Interface contract §5.1 lists only six sub-configs and omits this one,
       yet these source fields have no other home and T4 needs them. Flagged
       for commander sign-off as a contract addition, same as ``AssetsCfg``.
    """
    ...
    # Table reset geometry (cfg:378-386).
...
class TerminationCfg:
    ...
    # Tolerance curriculum — the only curriculum in v1 (cfg:442-444). T4 owns it.
...
class DomainRandomizationCfg:
    ...
    ``ResetPlan.randomization`` stays ``None`` (MIGRATION_00 decision D5).
    """
...
class SimToolRealCfg(EnvCfg):
    ...
    Frequencies follow MIGRATION_00 decision D4: ``sim_dt=1/120`` and
    ...
    def validate(self) -> None:
        """Validate frequency and episode-length consistency.

        Raises:
            ValueError: If ``sim_dt > ctrl_dt`` (base class), if the derived
                substep count is not the source's ``decimation=2``, or if the
                derived step budget disagrees with
