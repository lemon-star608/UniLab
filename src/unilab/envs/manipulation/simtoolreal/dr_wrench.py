...
(``action_utils.py:77-130``).  The public signature is intentionally stable
for the task's domain-randomization owner.
...
* Backend call: ``backend.apply_body_wrench`` through the shared backend
  contract.
"""
...
def sample_log_uniform(lo: float, hi: float, n: int) -> np.ndarray:
    ...
    return np.exp(np.random.uniform(log_lo, log_hi, size=(n,))).astype(np.float32)
