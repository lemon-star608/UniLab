"""SimToolReal action pipeline.

numpy port of ``isaacsimenvs/tasks/simtoolreal/utils/action_utils.py:18-75``
(``apply_action_pipeline``). The env boundary uses ``float32`` NumPy arrays;
``.clone()`` becomes ``.copy()``, ``torch.clamp`` becomes ``np.clip``, and
``torch.roll`` becomes ``np.roll``.
...
The task contract fixes ``dof_speed_scale=1.5``,
...
The owner clamps policy actions to ``[-clip_actions, clip_actions]`` before
permutation and delay; the shipped value is ``1.0``.
"""
...
from .delay_buffer import push_and_sample_delay
...
def apply_action_pipeline(env: Any, actions: np.ndarray) -> None:
    ...
    Args:
        ...
        actions: Policy actions of shape ``(num_envs, 29)`` in canonical joint
            order, clamped to ``[-clip_actions, clip_actions]`` here.
    ...
    # Clamp before the control law; the shipped owner sets ``clip_actions=1.0``.
    clip = float(act_cfg.clip_actions)
    ...
    if dr.use_action_delay and int(dr.action_delay_max) > 0:
        env._action_queue, actions_backend = push_and_sample_delay(
            env._action_queue,
            actions_backend,
            env,
