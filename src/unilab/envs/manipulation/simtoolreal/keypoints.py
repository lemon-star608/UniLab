"""Keypoint geometry for SimToolReal (migration task T5).
...
* observation path (per object, carries the scale DR)::
      ...
      size = phi * cfg.reward.object_base_size * object_scale_multiplier
...
* reward / success path (fixed across the whole object pool)::
      ...
      size = cfg.reward.fixed_size

  T0 already folded that into ``env._keypoint_offsets_fixed`` (a ``(4, 3)``
...
def compute_keypoints_from_offsets(
    ...
    on the reward/success path, which T0 built per reset_utils.py:80-83.
...
def compute_keypoints(
    ...
    *,
    ...
    size such as ``cfg.reward.fixed_size`` on the reward path.
...
def keypoint_max_dist(obj_kp: np.ndarray, goal_kp: np.ndarray) -> np.ndarray:
    ...
    (obs_utils.py:195), not the raw tolerance. That gate is T4's.
...
def observation_keypoint_size(
    ...
    Args:
        ...
        object_base_size: ``cfg.reward.object_base_size`` (source cfg:338, 0.04).
