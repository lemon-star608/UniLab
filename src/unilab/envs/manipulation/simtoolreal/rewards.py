"""Reward computation for SimToolReal (migration task T3).
...
``scale_value: 0.01`` (cfg/train/SimToolRealSAPG.yaml:77) is T9's job, not T3's.
...
def update_near_goal_steps(
    ...
    Ported from reward_utils.py:67-76. Used by T4's success logic, not directly
    by the reward.
...
def compute_rewards(env: SimToolRealEnv, info: dict[str, np.ndarray]) -> np.ndarray:
