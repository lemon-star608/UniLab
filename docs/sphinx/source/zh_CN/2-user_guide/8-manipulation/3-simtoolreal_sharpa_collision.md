...
正式 SimToolReal MuJoCo 场景原来使用 Mesh Sharpa collision mesh。本轮对照实验迁移后，
Sharpa collision geometry 切换为 Menagerie Sharpa：palm 使用 palm000.obj 到
...
margin=0、gap=0；visual geom 使用 contype=0、conaffinity=0、group=2。本轮没有
...
本轮没有修改 reward、reset、goal sampling、domain randomization、episode length、
...
common friction 仍为 0.5 0.005 0.0001；没有把 common friction profile 作为本轮的第二个
owner，也没有修改 AssetsCfg 或 tool_assets.py 的 friction。
